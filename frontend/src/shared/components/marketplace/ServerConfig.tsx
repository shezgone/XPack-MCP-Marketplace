...
export const ServerConfig: React.FC<ServerConfigProps> = ({
  ...
  const userToken = useSharedStore((state) => state.user_token);
  const isVisitor = !!visitor;
  const hasAccess = !!can_invoke;
  const canShowConfig = !isVisitor && hasAccess;
  const displayName = user?.nick_name || user?.user_name || user?.user_email;

  const fetchApiKeys = useCallback(
    async (): Promise<apiKeyList[]> => {
      try {
  ...
  return (
    <div className={`min-w-full ${className} flex-shrink-0`}>
      <div className="mb-2 mt-1">
        <div className="w-full rounded-lg border border-white/10 bg-black/80 p-4 shadow-lg">
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs uppercase tracking-[0.18em] text-default-400">
                {t("Access Status")}
              </p>
              <p className="mt-1 text-sm text-white">
                {isVisitor
                  ? t("Sign in to view the MCP connection link for this service.")
                  : hasAccess
                    ? t("You can connect to this MCP service with your authorization key.")
                    : t("You don't have permission to access this MCP server.")}
              </p>
            </div>
            <Chip
              className="text-white"
              color={isVisitor ? "warning" : hasAccess ? "success" : "danger"}
              variant="flat"
            >
              {isVisitor
                ? t("Visitor")
          ...
          {!isVisitor && displayName ? (
            <p className="mt-3 text-xs text-default-400">
              {t("Signed in as")}: <span className="text-white">{displayName}</span>
            </p>
          ) : null}
          ...
          <div className="mt-4">
            {!userToken ? (
              ...
              <div className="w-full rounded-lg bg-[#F3126033] p-3 text-center">
                <p className="text-[#C20E4D] font-bold text-sm leading-relaxed">
                  {t("Request access from an administrator to view the MCP endpoint.")}
                </p>
              </div>
            )}
          </div>
