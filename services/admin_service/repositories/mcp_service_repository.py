...
class McpServiceRepository:
    ...
    def _apply_service_type_filter(self, query, service_type: Optional[str]):
        if not service_type:
            return query
        if service_type == "flowise":
            return query.filter(McpService.service_type.in_(["flowise", "flowise1"]))
        return query.filter(McpService.service_type == service_type)
    ...
    def get_all(self, keyword: Optional[str] = None, service_type: Optional[str] = None) -> List[McpService]:
        ...
        query = self._apply_service_type_filter(query, service_type)
        return query.all()
